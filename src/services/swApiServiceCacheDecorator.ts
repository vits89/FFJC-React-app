...
export async function swApiServiceCacheDecorator<T>(endpoint: string, classType: { new(): T }, number?: number):
  Promise<T[]> {
  ...
  const data = await swApiServiceTransformer<T>(endpoint, classType, number);
