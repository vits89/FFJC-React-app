        ...
        <TableHead>
          <TableRow>
            ...
            {Object.entries(columnNames).map(([propName, name]) => (
              <TableCell align="right" key={propName}>
                {name}
              </TableCell>
            ))}
            {deleteRow && <TableCell align="center">Actions</TableCell>}
          </TableRow>
        </TableHead>
        <TableBody>
          {rowData.map((data, index) => (
            <TableRow hover key={data.id || index}>
              <TableCell component="th" scope="row" key={`number.${index}`}>
                {index + 1}
              </TableCell>
                  ...
                  {(typeof data[propName] === 'boolean' && (
                    ...
                    (data[propName] && propName === 'name' && (
                      <Link
                        ...
                        to={`${routeMatch.path}/${data.id || index}`}
                      >
                        {data[propName]}
                      </Link>
                    )) ||
              ...
              {deleteRow && (
                <TableCell align="center" key={`delete.${index}`}>
                  <Button
                    variant="contained"
                    color="secondary"
                    onClick={() => deleteRow(data.id || index)}
                  >
                    Delete
                  </Button>
                </TableCell>
