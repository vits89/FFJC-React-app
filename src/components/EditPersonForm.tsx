...
export const EditPersonForm: FunctionComponent<EditPersonComponentProps> = ({
  getInitialData,
  saveData,
}) => {
  ...
  const initialData =
    (id && getInitialData && getInitialData(id)) || new Person();
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      gender: Yup.mixed()
        .oneOf(['male', 'female', 'n/a'] as const)
        .required(),
  ...
  return (
    <>
      <Typography component="h3" variant="h6">
        {initialData.id ? 'Edit' : 'Add'} person
      </Typography>
      <Box marginY={1}>
        ...
        <FormControl fullWidth>
          <FormControlLabel
            ...
            control={
              <Checkbox
                color="default"
                checked={formik.values.beloved}
                {...formik.getFieldProps('beloved')}
              />
            }
          />
        </FormControl>
